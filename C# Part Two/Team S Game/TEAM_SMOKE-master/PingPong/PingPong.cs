...
namespace PingPong
{
    internal class PingPong
    {
        ...
        private static ConsoleColor ColorOfPaddle = ConsoleColor.White;
        private static readonly char symbolPaddle = '_';
        private static readonly char symbolPaddle2 = '*';
        private static readonly int PaddleLength = 20;
        private static int PaddlePositionX = width/2 - PaddleLength/2;
        private static int PaddlePositionY = height - 4;
        private static int BallPosX = width/2 - 5;
        private static int BallPosY = height/2 + 5;
        private static int BallPosX2 = width/2;
        private static int BallPosY2 = height/2;
        private static bool BallDirectionUp = true;
        private static bool BallDirectionUp2 = true;
        private static bool BallDirectionRight = true;
        private static bool BallDirectionRight2 = true;
        ...
        private static void MoveBall2() // Peter Boev
        {
        ...
        private static void PrintAtPosition(int x, int y, char symbol, ConsoleColor color) // Nikk-Dzhurov
        {
        ...
        private static void Logo(int posX, int posY) // Nikk-Dzhurov
        {
            ...
            while (tmp < 5)
            {
                ...
                for (var i = 9; i < matrix.GetLength(0); i++)
                {
                    for (var j = 0; j < matrix.GetLength(1); j++)
                    {
                        ...
                        else if (matrix[i, j] == '-' || matrix[i, j] == '_' || matrix[i, j] == '\\' ||
                                 matrix[i, j] == '/')
                        {
        ...
        private static void ConsoleView() //Niya Keranova
        {
            Console.ForegroundColor = ConsoleColor.White;
        ...
        private static void Startup() //Niya Keranova
        {
            Console.ForegroundColor = ConsoleColor.DarkGreen;
            var title = "PING-PONG GAME";
            Console.CursorLeft = Console.BufferWidth/2 - title.Length/2;
            Console.WriteLine(title);
            var longestString = "Rightarow (->) - Right";
            var cursorLeft = Console.BufferWidth - longestString.Length*2 - 1;
            Console.CursorTop = 5;
            Console.CursorLeft = cursorLeft;
            var oneRow = "Player's controls:";
            Console.WriteLine(oneRow);
            Console.ForegroundColor = ConsoleColor.Green;
            Console.CursorLeft = cursorLeft;
            var twoRow = "Leftarow (<-) - Left";
            Console.WriteLine(twoRow);
            Console.CursorLeft = cursorLeft;
            var threeRow = "Rightarow (->) - Right";
            Console.WriteLine(threeRow);
            Console.ReadKey();
            Console.Clear();
        }

        private static void GreatingsFromTheTeam() //Niya Keranova + Nikk Dzhurov
        {
            Console.ForegroundColor = ConsoleColor.DarkGreen;
            ...
            foreach (string item in greatingsTitle)
            {
                Console.SetCursorPosition(x, y++);
                Console.Write(item);
                Thread.Sleep(100);
        ...
        private static void Greatings() //Niya Keranova
        {
            Console.ForegroundColor = ConsoleColor.DarkGreen;
            var greatings = "REPRESENT: PING-PONG GAME";
            var y = Console.WindowHeight/2;
            var x = Console.WindowWidth/2 + 10 - greatings.Length;
            Console.SetCursorPosition(x, y);
        ...
        private static void Loading() //Niya Keranova
        {
            var i = 0;
            while (i != 7)
            {
        ...
        private static void PrintPaddlePossition(int x, int y, char symbolPaddle, int length, ConsoleColor color)
        {
            Console.ForegroundColor = color;
            Console.SetCursorPosition(x, y);
        ...
        private static void ClearBox() // Nikk-Dzhurov
        {
            for (var i = 1; i < Console.WindowHeight - 2; i++)
            {
        ...
        private static void PlayerMoveLeft(int padPositionX)
        {
        ...
        // Nikola Pineda + Emanuela
        private static void PlayerMoveRigth(int padPositionX)
        {
        ...
        // Nikola Pineda + Emanuela
        private static void MovePaddle()
        {
            ...
            using (reader)
            {
                ...
                while (line != null)
                {
                    if (!string.IsNullOrEmpty(line))
                    {
                        ...
                        var score = int.Parse(splitLine[1]);
        ...
        {
            var soundWall = new SoundPlayer(); // Chernogorov
        ...
        {
            var soundLive = new SoundPlayer(); // Chernogorov
        ...
        {
            var soundGameOver = new SoundPlayer(); // Chernogorov
        ...
        {
            var soundJumpBall = new SoundPlayer(); // Chernogorov
        ...
        private static void ChoiceMenu() //Chernogorov
        {
            if (BallPosY > PaddlePositionY || BallPosY2 > PaddlePositionY) //Chernogorov
            {
                ...
                var keyOne = Console.ReadKey(); //Chernogorov
                ...
                if (keyOne.KeyChar == 's') //TodorDimitrov
                {
                    ...
                    if (database.Count > 0)
                    {
                        ...
                        {
                            var name = player.Key;
                            var score = player.Value.ToString().PadLeft(Console.WindowWidth - name.Length, '-');
        ...
        private static void PrintPoints() // Peter Boev
        {
            Console.SetCursorPosition(Console.WindowWidth/2 - 10, 0);
            Console.Write("Points : {0}", points);
        }

        private static void PrintLevel() // Peter Boev
        {
        ...
        private static void PrindSecondBall() // Peter Boev
        {
